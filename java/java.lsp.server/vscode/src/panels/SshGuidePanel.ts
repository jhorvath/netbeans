...
export class SshGuidePanel {
    public static currentPanel: SshGuidePanel | undefined;
    public static readonly viewType: string = "sshGuide";

    private static readonly actionType: NotificationActionType = 'ssh-connection';
    ...
    public static createOrShow(context: vscode.ExtensionContext) {
        ...
        SshGuidePanel.currentPanel = new SshGuidePanel(context);
    }

    private constructor(context: vscode.ExtensionContext) {
        this.panel = vscode.window.createWebviewPanel(
            SshGuidePanel.viewType,
            "Oracle Cloud Assets",
            { viewColumn: vscode.ViewColumn.One, preserveFocus: true },
            {
                enableCommandUris: true,
                enableScripts: true,
                localResourceRoots: [vscode.Uri.file(path.join(context.extensionPath, SshGuidePanel.webviewsFolder))],
            }
        );
        ...
        this.panel.webview.onDidReceiveMessage(
            (message) => {
                if (message.command === "showGuide") {
                    ...
                    toggleGuideFor(SshGuidePanel.actionType, ocid);
                }
            },
            undefined,
            this.disposables
    ...
    private async setHtml(context: vscode.ExtensionContext) {
        let templateFile = path.resolve(__dirname, "../../webviews/ssh-guide.handlebars");
        ...
        this.panel.webview.html = template({
            ...
            homeDir: os.homedir(),
            showGuide: shouldShowGuideFor(SshGuidePanel.actionType, ocid) ? "checked" : "",
            cssUri: this.panel.webview.asWebviewUri(vscode.Uri.file(path.join(context.extensionPath, SshGuidePanel.webviewsFolder, SshGuidePanel.stylesFolder, "guide.css"))),
            javascriptUri: this.panel.webview.asWebviewUri(
                vscode.Uri.file(path.join(context.extensionPath, SshGuidePanel.webviewsFolder, SshGuidePanel.scriptsFolder, "guide.js"))
            ),
        });
    }
